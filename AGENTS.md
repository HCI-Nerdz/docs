...
- Context Edge: `context-edge.adoc` (slug: context-edge) — edge-summoned ecosystem nav (top/left rails → wireframe platform map; overlay colors); essay `/blog/when-platforms-overload-the-entrypoint/`; demo `/demos/context-edge/`; MVP `HCI-Nerdz/context-edge`
