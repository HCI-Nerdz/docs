...
In plain language: install once, show up in every file manager — the same honesty Desktop Entry Spec already gives application launchers.
...
Profiles may constrain `MimeTypes`, `Schemes`, and `SelectionCount` so background vs selection stay honest.
...
* xref:pass-through-extensions.adoc[Pass-through extensions] — association honesty for meta-suffixes
