...
:page-last-edited: 2026-09-10T03:00-0500
...
image::actor-model-agentic-ui/node-graph-topology.svg[alt="Node graph with spawn edges, mailboxes, and grid projection",role=themed-svg,width=720]

One graph; multiple projections over the same disk store:

* **Cadence separation** — main and discussion chat carry coordinator voice and planning; worker agents do not flood the scroll. Workers appear as tiles and status chips; fork a node for the full `chat.jsonl` transcript.
* **Linear** — discussion and coordinator nodes in a sidebar; main thread is discussion only; workers as compact PM status, not transcript lines.
* **Grid** — `type=task` nodes as parallel tiles with job status + summary; separate scroll from main thread; archive bin for completed work.
* **Tree** — left-to-right spawn lineage (`spawnedBy` / `spawned`); spawn-on-drift without polluting the parent log.
...
* **Temporal layout** — scrubber spans linear, grid, and tree to rewind activity across parallel nodes (see below).
...
Parallel nodes do not naturally chapter together. **Temporal layout** is a view engine — same component, different `scope`; the live demo exposes a scrubber control across linear, grid, and tree modes:
...
link:https://hci-nerdz.github.io/demos/actor-model-agentic-ui/[Live demo] — linear (discussion sidebar + worker status), grid (parallel task tiles), tree (spawn lineage with + child), fork + title index, epochs, temporal scrubber; serialized wall shown as the problem in the lede only.
...
* link:https://hci-nerdz.github.io/blog/persisting-agent-work-as-a-node-graph/[Essay — Persisting agent work as a node graph]
