...
:description: Diagnosis and treatment — an alternative to URL-as-identity. Pretty URLs stay for people; systems wire on content identity, mutable pointers, and reverse consumers.

image::navigating-by-content/icon.svg[An alternative to URLs — hash core with detachable URL label,width=120]
...
| link:https://hci-nerdz.github.io/blog/an-alternative-to-urls/[An alternative to URLs] · this topic
...
Slug stays `navigating-by-content` (Cool URI absorb). Public title is *An alternative to URLs*; internal nickname after the hook is *Labels versus wires*.
Do not confuse with the instruction-flows essay *Navigating by content* (`/blog/navigating-by-content/`) — menu-style docs, not URL identity.
...
*An alternative to URLs* (nickname: *labels versus wires*) is the claim that **identity should follow the thing**, not the pretty name we hang on it today.
